...
export function SubMenuItem({ href, label, icon, description }: SubMenuItemProps) {
  return (
    <Link
      href={href}
      className={`group flex items-start gap-4 p-4 rounded-lg hover:bg-[#f0f9f0] dark:hover:bg-[#1a2a1a] 
    ...
    >
      ...
      <div className="flex flex-col gap-1">
        <span className="font-medium group-hover:text-hub-primary transition-colors duration-200">
          {label}
        </span>
        {description && (
          <span className="text-sm text-hub-text-secondary leading-relaxed group-hover:opacity-90 transition-opacity duration-200">
            {description}
          </span>
        )}
      </div>
